...
namespace SeismicStomp
{
    public class SeismicStompMain : ModBase, IOnGameExit, IOnHeroUpdate
    {
        // ---------- 关卡深度映射 ----------
        ...
        // ---------- 构造函数 ----------
        public SeismicStompMain(ModInfo info) : base(info)
        {
        }
        ...
        {
            base.Initialize();
            System.Console.WriteLine("✅ SeismicStompMain 初始化完成，按 N 传送上一关，按 M 传送下一关，按 K 切换方向反转");
        }
        ...
        void IOnHeroUpdate.OnHeroUpdate(double dt)
        {
            ...
            // 检测 K 键（切换方向反转）
            bool isKPressedNow = GetAsyncKeyState(VK_K) < 0;
            if (isKPressedNow && !_isKKeyPressed)
            {
                ToggleInvertMovement();
            }
            _isKKeyPressed = isKPressedNow;
        }
        ...
        {
            System.Console.WriteLine("游戏退出，SeismicStompMain 资源清理");
        }
    }
}
