...
public class HeroU28VacuumCleaner : U28_VacuumCleaner
{
	private int jhf;
	public static HeroU28VacuumCleaner? inst { get; private set; }
	public Dictionary<string, KeyBind> keys => PlayableMOB.config.Value.enforcer.bindings;
	private OldMobSkill? s1, s2, s3, s4;
	...
	public HeroU28VacuumCleaner(Level l, int x, int y, int d, int lt) : base(l, x, y, d, lt) { }

	public void playerInit()
	{
		Utils.mobInit((dc.en.Mob)this);
		if (base.oldSkills != null) { dynamic s = base.oldSkills; int t = ((dynamic)s).length;
			try { s1 = (OldMobSkill)(dynamic)s.getDyn(t - 4); } catch { }
			try { s2 = (OldMobSkill)(dynamic)s.getDyn(t - 3); } catch { }
			try { s3 = (OldMobSkill)(dynamic)s.getDyn(t - 2); } catch { }
			try { s4 = (OldMobSkill)(dynamic)s.getDyn(t - 1); } catch { }
		}
