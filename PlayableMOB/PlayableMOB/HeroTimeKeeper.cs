...
public class HeroTimeKeeper : TimeKeeper
{
	...
	public static void create(Hero h)
	{
		if (inst == null)
		{
			var m = new HeroTimeKeeper(dc.pr.Game.Class.ME.curLevel, ((Entity)h).cx, ((Entity)h).cy, 38, 38);
			((Entity)m).dir = ((Entity)h).dir;
			Utils.bossInit((dc.en.mob.Boss)m);
		((Entity)m).isOutOfGame = false;
		m.playerInit();
		}
	}
	public HeroTimeKeeper(Level lv, int x, int y, int dt, int lt) : base(lv, x, y, dt, lt) { }
