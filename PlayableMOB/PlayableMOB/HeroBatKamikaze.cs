...
public class HeroBatKamikaze : BatKamikaze
{
	private int jhf;
	public static HeroBatKamikaze? inst { get; private set; }
	public Dictionary<string, KeyBind> keys => PlayableMOB.config.Value.enforcer.bindings;
	private OldMobSkill? s1, s2;

	public static void create(Hero h)
	{
		if (inst == null)
		{
			var m = new HeroBatKamikaze(dc.pr.Game.Class.ME.curLevel, ((Entity)h).cx, ((Entity)h).cy, Utils.DamageTier(), Utils.LifeTier());
			((Entity)m).dir = ((Entity)h).dir;
			((Entity)m).init(); m.playerInit();
		}
	}
	public HeroBatKamikaze(Level l, int x, int y, int d, int lt) : base(l, x, y, d, lt) { }

	public void playerInit()
	{
		Utils.mobInit((dc.en.Mob)this);
		if (base.oldSkills != null) { dynamic s = base.oldSkills; int t = ((dynamic)s).length;
			try { s1 = (OldMobSkill)(dynamic)s.getDyn(t - 2); } catch { }
			try { s2 = (OldMobSkill)(dynamic)s.getDyn(t - 1); } catch { }
		}
