...
public class HeroAxeStatue : AxeStatue
{
	private int jhf;
	public static HeroAxeStatue? inst { get; private set; }
	...
	public static void create(Hero h)
	{
		if (inst == null)
		{
			var m = new HeroAxeStatue(dc.pr.Game.Class.ME.curLevel, ((Entity)h).cx, ((Entity)h).cy, Utils.DamageTier(), Utils.LifeTier());
			((Entity)m).dir = ((Entity)h).dir;
			((Entity)m).init(); m.playerInit();
		}
	}
	public HeroAxeStatue(Level l, int x, int y, int d, int lt) : base(l, x, y, d, lt) { }
