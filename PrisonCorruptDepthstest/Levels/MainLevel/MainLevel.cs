...
namespace PrisonCorruptDepthstest.Levels.MainLevel;

public class MainLevel : ILevel
{
    public static ModCore.Storage.Config<MainLevelConfig> Config { get; } = new("MainLevelConfig");
    ...
    {
        return new MainLevelStruct(user, levelData, rng);
    }

    public dc.level.LevelDisp CreateLevelDisplay(dc.pr.Level level, dc.level.LevelMap map)
    {
        // Reuses Prison display via LevelManager routing
        return null!;
    }
