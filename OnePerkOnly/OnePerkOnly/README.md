...
**效果叠加（默认开启）**：同一变异拿 N 份时，该变异“随卷轴成长”类效果（公式 `prct + customScaling×(tier−1)`）会被放大 N 倍 —— 通过 Hook `Entity.getRelevantPerkTier` 返回等效卷轴数实现，对 P_AttackSpeed_Combo 这类按卷轴缩放属性的变异是精确 ×N。

> 限制说明：游戏引擎的变异效果都是“按 id 布尔判断 + 按卷轴缩放”，本身不读持有份数；因此只有**走 `getRelevantPerkTier` 随卷轴成长的变异**能精确叠加。固定数值效果（customScaling=0，如部分一次性/固定值变异）无法用等效 tier 放大，日志里会提示“叠加跳过”。没有通用办法让任意变异都 ×N（需要逐条改游戏内每个效果公式）。
