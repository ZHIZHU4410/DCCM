...
namespace chuanqiang_canying
{
    public class chuanqiang_canyingMain : ModBase, IOnGameExit, IOnHeroUpdate, IOnGameEndInit
    {
        // ---------- 颜色转换辅助方法 ----------
        ...
        public chuanqiang_canyingMain(ModInfo info) : base(info) { }
        ...
        public override void Initialize()
        {
            ...
            System.Console.WriteLine("chuanqiang_canyingMain 初始化完成");
        }
        ...
        void IOnHeroUpdate.OnHeroUpdate(double dt)
        {
            ...
            // ---------- 默认自带残影：按时间间隔生成随机颜色残影 ----------
            Hero hero = ModCore.Modules.Game.Instance.HeroInstance;
            if (hero != null && hero._level != null)
            {
                _trailAccumulator += dt;
                if (_trailAccumulator >= TrailInterval)
                {
                    _trailAccumulator -= TrailInterval;
                    CreateRandomTrail(hero);
                }
            }
            else
            {
                // 英雄不存在时重置累加器，避免重新进入时立即爆发多个残影
                _trailAccumulator = 0;
            }
        }
        ...
        {
            System.Console.WriteLine("游戏退出，chuanqiang_canyingMain 资源清理");
        }
    }
}
